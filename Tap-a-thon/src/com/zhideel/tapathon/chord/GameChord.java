...
public abstract class GameChord extends AbstractChord {
	...
	void handlePrivateMessage(ChordMessage message) {
		...
		case DEALER:
			...
			break;
		case BLIND:
			//mBus.post(new ClientModelEvent.BlindEvent(message.getInt(ChordMessage.AMOUNT), (BlindType) message.getObject(ChordMessage.BLIND_TYPE)));
			break;
		case CARD_PAIR:
			//final Pair<Card, Card> cardPair = new Pair<Card, Card>((Card) message.getObject(ChordMessage.FIRST_CARD), (Card) message.getObject(ChordMessage.SECOND_CARD));
