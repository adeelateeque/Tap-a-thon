...
public class MultiTouchView extends View {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_POINTER_DOWN: {
			ArrayList<Integer> operands = ((GamePadActivity) super.getContext()).getOperands();
			String operator = ((GamePadActivity) super.getContext()).getOperator();
			
			try {
				int number = Integer.parseInt(currentText);
				if (operands.size() < 2){
					((GamePadActivity) super.getContext()).addOperand(number);
					this.isSelected = true;
					if ((operands.size() == 2) && (operator != null)) {
						int result = ((GamePadActivity) super.getContext()).doCalc();
						Toast.makeText(getContext(), Integer.toString(result), Toast.LENGTH_SHORT).show();
						((GamePadActivity) super.getContext()).resetCurrent();
					}
				}
				...
				if (operator == null){
					((GamePadActivity) super.getContext()).setOperator(cOperator);
					this.isSelected = true;
					if (operands.size() == 2) {
						int result = ((GamePadActivity) super.getContext()).doCalc();
						Toast.makeText(getContext(), Integer.toString(result), Toast.LENGTH_SHORT).show();
